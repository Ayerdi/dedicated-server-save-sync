...
usage() {
  cat <<'EOF'
Usage:
  config/deploy.sh             Deploy using an existing .env.
  config/deploy.sh --init-env  Create .env safely and exit without deploying.
EOF
}
...
init_env() {
  ...
  [[ -f "${example_path}" ]] || { log "Missing ${example_path}."; return 1; }
  if [[ -e "${env_path}" ]]; then
    log "${env_path} already exists; it was not overwritten."
    return 1
  fi
...
for key in keys:
    ...
    if len(pattern.findall(source)) != 1:
        raise SystemExit(f".env.example must contain exactly one line {key}")
...
try:
    ...
    # link() is atomic and fails if another process already created .env; it never overwrites.
...
PY
  ...
  log "Created ${env_path} with mode 0600 and random local secrets."
  log "Review non-secret values before running the deployment."
}

restore_route_on_error() {
  local exit_code=$?
  if (( route_published )); then
    log "Verification failed; restoring the previous Traefik route."
...
[[ -f "${PROJECT_DIR}/.env" ]] || { log "Missing ${PROJECT_DIR}/.env; start from .env.example."; exit 1; }
env_value() {
...
[[ "${GAME_KEY}" =~ ^[a-z0-9][a-z0-9-]{0,62}$ ]] || { log "SAVE_SYNC_GAME_KEY is invalid."; exit 1; }
case "${PANEL_MODE}" in
  ...
  *) log "SAVE_SYNC_PANEL_MODE must be authentik or disabled."; exit 1 ;;
esac
[[ -f "${TEMPLATE_PATH}" ]] || { log "Missing Traefik template."; exit 1; }
[[ -n "${TRAEFIK_DYNAMIC_DIR}" ]] || { log "Missing SAVE_SYNC_TRAEFIK_DYNAMIC_DIR."; exit 1; }
[[ -n "${PUBLIC_BASE_URL}" ]] || { log "Missing SAVE_SYNC_PUBLIC_BASE_URL."; exit 1; }
GAME_CONFIG_PATH="${PROJECT_DIR}/config/games/${GAME_KEY}.json"
[[ -f "${GAME_CONFIG_PATH}" ]] || { log "Missing ${GAME_CONFIG_PATH}."; exit 1; }
...
[[ -d "${TRAEFIK_DYNAMIC_DIR}" ]] || { log "${TRAEFIK_DYNAMIC_DIR} does not exist."; exit 1; }

mkdir -p "${RUNTIME_DIR}"
chmod 0700 "${RUNTIME_DIR}"

log "Validating private configuration and generating the runtime route."
...
if [[ "${env_mode}" != "600" && "${env_mode}" != "400" ]]; then
  log "The .env file must have mode 0600 or 0400 (current: ${env_mode})."
  exit 1
fi

cd "${PROJECT_DIR}"
log "Validating and building the isolated stack."
docker compose -p "${COMPOSE_PROJECT}" config --quiet
docker compose -p "${COMPOSE_PROJECT}" build --pull

log "Preparing the private volume with UID 10001."
# These capabilities are granted only to the ephemeral setup container. CHOWN changes
# ownership; DAC_OVERRIDE allows traversing a UID-10001 0700 tree during redeployments,
# and FOWNER allows correcting file modes. The final service keeps cap_drop: ALL in
# docker-compose.yml.
...
log "Starting backend and durable supervisor for ${GAME_KEY}."
...
[[ "${health}" == "healthy" ]] || { log "Backend is not healthy: ${health}."; exit 1; }
...
[[ "${backup_health}" == "healthy" ]] || {
  log "Backup supervisor is not healthy: ${backup_health}."
...
log "Publishing the Traefik route with an atomic rename."
...
route_published=0
trap - ERR
log "Deployment verified. Backend and supervisor are healthy; Traefik was not restarted."